...
Game::Game(GameMode *mode) {
  ...
  if (this->gameMode->getModeName() == "normal" ||
      this->gameMode->getModeName() == "multiplayer" ||
      this->gameMode->getModeName() == "simulation") {
    ...
    for (int i = 0; i < playerNum; i++) {
      ...
      while (true) {
        ...
        // Regex to allow only letters, numbers, and underscores (change as
        // needed)
        if (!regex_match(playerName, regex("^[a-zA-Z0-9_]+$"))) {
          cout << "Name must contain only letters, numbers, or underscores. "
                  "Try again.\n";
          continue;
        }
...
void Game::start() {
  ...
  if (!this->isClockwise()) this->currentPlayerIndex = 3;
...
bool Game::isValidMove(Card *card) {
  return card->canPlayOn(this->currentCard) ||
         card->get_Color() == this->currentColor;
}
...
void Game::specialActionCheck() {
  ...
  if (!currentPlayer) {
    std::cerr << "[ERROR] currentPlayer is nullptr in specialActionCheck."
              << std::endl;
    return;
  }
  ...
  if (reverse) {
    this->skipPlayer();
  } else if (skip && ((this->currentCard->get_ActionType() != Skip &&
...
void Game::loadScores() {  // Shows current log of players names' and their
                           // respective scores
  std::ifstream infile("scores.txt");

  if (!infile.is_open()) {
    std::cerr << "scores could not be found or opened; empty scoresheet."
              << std::endl;
    return;
  }
...
void Game::saveScores() {  // Overwrite score of the new winner
  ...
  if (!outfile.is_open()) {
    std::cerr << "scores could not be opened or created; scores not saved."
              << std::endl;
  }
...
void Game::updateScores(std::string winnerName) {
  ...
  std::cout << winnerName << " gains " << points << " points this round!"
            << std::endl;
}
